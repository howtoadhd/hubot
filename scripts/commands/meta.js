module.exports = robot => {
  ...
  robot.respond(/META PING$/i, msg => msg.send("PONG"));

  robot.respond(/META ADAPTER$/i, msg => msg.send(robot.adapterName));

  robot.respond(/META ECHO (.*)$/i, msg => msg.send(msg.match[1]));

  robot.respond(/META TIME$/i, msg => msg.send(`Server time is: ${new Date()}`));
};
